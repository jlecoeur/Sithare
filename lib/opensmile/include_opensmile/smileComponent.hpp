...
public:
  ...
  cComponentMessage(const char *_type=NULL, const char *_name=NULL) :  sender(NULL), smileTime(0.0), readerTime(-1.0), msgid(-1), userflag1(0), userflag2(0), userflag3(0), custData(NULL), custData2(NULL) {
...
class DLLEXPORT cSmileComponent {
  ...
  public:
    ...
    int tick(long long t, int _EOI=0) {
      if (_EOI) {
        if (!isEOI()) {
          setEOI();
