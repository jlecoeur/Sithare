...
string Lyrics::saveLyrics(string lyricsPath)
{
	string fileName;
	...
	fileName = lyricsPath + "/" + artistURL + "-" + titleURL + ".lyr";
...
bool Lyrics::fetchLyrics()
{
	...
	else if(fetchSeekLyrics())
	{
		return true;
	}
	else if(fetchAliveLyrics())
	{
		return true;
	}
	//else if(fetchFrCopyLyrics())
	//{
	//	return true;
	//}
	// else if( ... )
...
string Lyrics::getSeekLyricsURL()
{
	string artistURL, titleURL;
	...
	// Get url
	string url = "http://www.seeklyrics.com/lyrics/";
	url = url + artistURL + "/" + titleURL;
...
bool Lyrics::fetchSeekLyrics()
{
   ...
   curl_easy_perform(curl);
	...
	// Post-Process lyrics
	// -----------------------
	found = buffer.find("<div id=\"songlyrics\">");
	if(found != string::npos)
	{
		// cut before the lyrics
		buffer = buffer.substr(found+29, string::npos);
...
string Lyrics::getAliveLyricsURL()
{
	string artistURL, titleURL;
	...
	// Get url
	string url = "http://www.alivelyrics.com/";
	url = url + artistURL + "/" + titleURL;
	//url = url + artistURL.substr(0,1) + "/" artistURL + "/" + titleURL;
...
bool Lyrics::fetchAliveLyrics()
{
   ...
   curl_easy_perform(curl);
	...
	// Post-Process lyrics
	// -----------------------
	found = buffer.find("<pre class=\"lyrics\">");
	if(found != string::npos)
	{
		// cut before the lyrics
		buffer = buffer.substr(found+29, string::npos);
		// cut after the lyrics
		found = buffer.find("</pre> ");
		buffer = buffer.substr(0,found);
		// remove <br/>
		replaceAll(buffer, "<br/>", "\n");
		// remove <p/>
...
string Lyrics::getFrCopyLyricsURL()
{
	string artistURL, titleURL;
	...
	// Get url
	string url = "http://fr.lyrics-copy.com/";
	url = url + artistURL + "/" + titleURL;
...
bool Lyrics::fetchFrCopyLyrics()
{
   ...
   curl_easy_perform(curl);
	...
	// Post-Process lyrics
	// -----------------------
	found = buffer.find("<div id=\"paroles\">");
	if(found != string::npos)
	{
		// cut before the lyrics
		buffer = buffer.substr(found+29, string::npos);
